...
表 [10-1](#Tab1) 列出了在 Java 应用中使用脚本引擎之前，如何安装脚本引擎的详细信息。网站列表和说明在撰写本文时仍然有效；它们可能在阅读时失效。但是，它们向您展示了脚本语言的脚本引擎是如何安装的。如果你对使用 Nashorn 感兴趣，你不需要在你的机器上安装任何东西。Nashorn 在 JDK 8 中可用。
...
![A978-1-4302-6662-4_10_Fig2_HTML.jpg](img/A978-1-4302-6662-4_10_Fig2_HTML.jpg)
...
表 [10-2](#Tab2) 包含所有保留键的列表。这些键在`ScriptEngine`接口中也被声明为常量。脚本引擎的实现不需要在引擎范围绑定中将所有这些键传递给引擎。作为开发人员，您不应该使用这些键将参数从 Java 应用传递到脚本引擎。
...
| `-doe or –dump-on-error` | 如果指定了此选项，将打印错误的完整栈跟踪。默认情况下，会打印一条简短的错误消息。 |
...
Nashorn 的命令行工具允许您从脚本中使用 JavaFX。您需要在 JavaScript 中创建一个`start()`函数，就像您在 Java 中启动 JavaFX 应用一样。纳森会处理剩下的事情。可选地，您也可以为您的 JavaFX 应用声明`init()`和`stop()`函数。您可以使用 JavaFX 类的完全限定名，或者使用`Java.type()`函数导入它们。下面的代码片段展示了在 JavaFX 中创建`Label`的两种方法:
...
清单 10-32 包含一个 JavaFX 应用的代码。将代码保存在名为`hellojavafx.js`的文件中。
...
这是在第 9 章的[中定义为`ImprovedHelloFXApp` Java 类的 JavaFX 应用的 Nashorn 脚本等价物。Nashorn 版本的代码编写起来稍微简单一点。在脚本中，您可以使用 Java 类的属性来调用它们的方法。例如，代替书写](09.html)
...
要运行 JavaFX 应用，您需要用一个`–fx`选项启动`jjs`工具。以下命令启动 JavaFX 应用，显示如图 [10-7](#Fig7) 所示的窗口。输入姓名并点击`Say Hello`按钮查看消息。点击`Exit`按钮退出应用。

`C:\> jjs -fx hellojavafx.js`

![A978-1-4302-6662-4_10_Fig7_HTML.jpg](img/A978-1-4302-6662-4_10_Fig7_HTML.jpg)

图 10-7。

A JavaFX window created using Nashorn script

`jjs`命令行工具使得使用 JavaFX 应用变得非常容易。只需一行代码就可以在 JavaFX 窗口中显示一条消息。Nashorn 创建了一个名为`$STAGE`的全局变量，它是 JavaFX 应用初级阶段的引用。请注意，只有在使用带有`–fx`选项的`jjs`工具时，脚本中的`$STAGE`全局变量才可用。清单 10-33 显示了最简单的 JavaFX 应用的代码，它在 JavaFX 窗口中显示一个带有消息的`Label`。将其保存在名为`simplestfxapp.js`的文件中。注意，您不必再为`start()`方法创建任何函数。您甚至不需要在`$STAGE`变量上调用`show()`方法。Nashorn 会自动显示舞台。
...
以下命令将运行最简单的 JavaFX 应用，该应用显示如图 [10-8](#Fig8) 所示的窗口。

![A978-1-4302-6662-4_10_Fig8_HTML.jpg](img/A978-1-4302-6662-4_10_Fig8_HTML.jpg)
...
Java 8 提供了两个命令行工具，分别叫做`jrunscript`和`jjs`。它们位于`JDK_HOME\bin`目录中。它们用于在命令行上运行脚本。`jrunscript`工具是脚本语言中立的；它可以用来执行 Nashorn、JRuby、groovy 等任何脚本语言的脚本。`jjs`工具用于运行 Nashorn 脚本及其扩展；您可以使用`jjs`工具运行 shell 命令、脚本和 Java 应用。