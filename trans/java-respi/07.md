...
`DigitalInputDevice`有几个构造器，除了一个之外，所有的构造器都使用内置的默认值(参见 Javadoc)来实现上述的一些或全部特征。它还有一个`Builder`嵌套类。 <sup>[6](#Fn6)</sup> 注意，diozero 为上拉电阻类型和事件触发类型定义了常数，用于构造器或与`Builder`一起使用。
