...
创建一个不可变的类比看起来要稍微复杂一些。我已经在本节中介绍了一些案例。这是另一个你需要小心的例子。假设你设计了一个不可变的类，它有一个引用类型的实例变量。假设它在其一个构造器中接受其引用类型实例变量的初始值。如果实例变量的类是可变类，则必须复制传递给其构造器的参数，并将副本存储在实例变量中。在构造器中传递对象引用的客户端代码可能会在以后通过同一引用更改该对象的状态。清单 [11-22](#PC74) 展示了如何正确实现`IntHolderWrapper3`类的第二个构造器。它包含注释的第二个构造器的实现的错误版本。
