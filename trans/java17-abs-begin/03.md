...
![img/463938_2_En_3_Figm_HTML.jpg](img/463938_2_En_3_Figm_HTML.jpg)根据需要声明一个模块，意味着当代码被编译时需要这个模块——通常称为*编译时*，当代码被执行时——通常称为*运行时*。如果一个模块只在运行时需要，**需要静态的**关键字用于声明依赖关系。现在只要记住这一点，当我们谈论 web 应用程序时，它就有意义了。
...
![img/463938_2_En_3_Figp_HTML.jpg](img/463938_2_En_3_Figp_HTML.jpg)想象一下:这就像有一辆汽车，但你无法启动它，因为点火锁芯不见了。从所有的意图和目的来看，它是一辆汽车，但它不能执行汽车的主要目的，即实际上带你去某个地方。您可以将`main`方法想象成点火锁圆柱体，JRE 将在其中插入钥匙以使您的应用程序运行。让我们将该方法添加到`HelloWorld`类中。
