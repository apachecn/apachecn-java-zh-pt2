...
`Character`类的一个对象包装了一个`char`值。该类包含几个在处理字符时很有用的常数和方法。例如，它包含`isLetter()`和`isDigit()`方法来检查字符是字母还是数字。`toUpperCase()`和`toLowerCase()`方法将一个字符转换成大写和小写。值得研究一下这个类的 API 文档。该类提供了一个构造器和一个工厂`valueOf()`方法来从`char`创建对象。使用工厂方法以获得更好的性能。`charValue()`方法返回对象包装的`char`。以下代码片段显示了如何创建`Character`对象以及如何使用它们的一些方法:
