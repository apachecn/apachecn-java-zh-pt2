...
λ表达式访问局部变量`msg`。在 lambda 表达式中访问的任何局部变量实际上都必须是 final。lambda 表达式试图修改其主体内的`msg`变量，这会导致编译时错误。
...
有时，一个函数可能从它的主体中调用它自己。这样的函数称为递归函数。λ表达式代表一个函数。然而，lambda 表达式不支持递归调用。如果需要递归函数，需要使用方法引用或者匿名内部类。
