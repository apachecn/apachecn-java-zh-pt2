...
通量<t>是Reactor反应流的主要入口点，类似于 RxJava 的可观测值。Mono <t>就像是一个通量，只不过是零到一个元素。单声道和通量乐器`org.reactivestreams.Publisher`。</t></t>
...
Reactor有优秀的在线[文档](https://projectreactor.io/docs/core/release/api/Reactor)供参考。
