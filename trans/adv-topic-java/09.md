...
![9781430266198_Fig09-03.jpg](images/9781430266198_Fig09-03.jpg)
...
15.  `lo` (`4`) is less than `hi` (`6`), so swap `num[lo]` and `num[hi]`, giving this:
    ...
    ![9781430266198_unFig09-10.jpg](images/9781430266198_unFig09-10.jpg)
...
在前面显示的`quicksort`版本中，在子列表被划分后，我们调用`quicksort`，左边部分跟着右边部分。在大多数情况下，这将工作得很好。然而，对于大的`n`，挂起的递归调用的数量可能会变得很大，以至于产生一个“递归堆栈溢出”错误。
...
举个例子，假设我们正在排序`A[1..99]`，第一个分割点是 40。假设我们使用的是`partition2`，它不会将轴心放到最终的排序位置。因此，我们必须对`A[1..40]`和`A[41..99]`进行排序以完成排序。我们将堆栈(41，99)并首先处理`A[1..40]`(较短的子列表)。

假设`A[1..40]`的分割点是 25。我们将堆栈(1，25)并首先处理`A[26..40]`。在这个阶段，我们在堆栈上有两个子列表—(41，99)和(1，25)—需要排序。试图对`A[26..40]`进行排序将导致另一个子列表被添加到堆栈中，等等。在我们的实现中，我们还会将较短的子列表添加到堆栈中，但这将被立即移除并进行处理。

这里提到的结果向我们保证，在任何给定的时间，栈上绝不会有超过 log <sub>2</sub> 99 = 7(取整)的元素。即使对于`n` = 1，000，000，我们也保证栈项数不会超过 20。

当然，我们必须自己操作堆栈。每个堆栈元素将由两个整数组成(比如说`left`和`right`，这意味着从`left`到`right`的列表部分仍有待排序。我们可以将`NodeData`定义如下:
...
我们将使用 4.3 节中的堆栈实现。我们现在根据前面的讨论写`quicksort3`。它显示为独立[程序 P9.4](#list4) 的一部分。这个程序从文件`quick.in`中读取数字，使用`quicksort3`对数字进行排序，并打印排序后的数字，每行 10 个。
...
我们还在`quicksort3`中添加了语句，以跟踪在任何给定时间堆栈上的最大项目数。当用于对 100000 个整数进行排序时，堆栈项目的最大数量是 13。这小于理论上的最大值，log <sub>2</sub> 100000 = 17，向上取整。
