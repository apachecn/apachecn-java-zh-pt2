...
当我们使用不总是在线的资源时，最好使用`while`语句。假设我们在一个不稳定的网络中为我们的应用使用一个远程数据库。第一次超时后，我们可以尝试直到成功，而不是放弃保存数据，对吗？这是通过使用一个`while`语句来完成的，该语句将不断尝试在其代码块中初始化一个连接对象。代码大致如清单 [7-27](#PC27) 所示。
