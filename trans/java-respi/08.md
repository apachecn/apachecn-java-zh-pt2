...
我从简单的开始。据我所知,“G”LabVIEW 驱动程序下载只包含可执行文件，没有源代码；这使得它对移植毫无用处。ROS 的 Python 库似乎是由与 Basicmicro 无关的人编写的；至少四年没有更新；简单看一下它就知道它和树莓派OS 的 Python 库没有太大区别，后者来自 Basicmicro 并受支持。因此，ROS Python 库不是一个好的候选。
