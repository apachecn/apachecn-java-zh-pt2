...
堆栈被实现为一个链表，在链表的头部添加新的条目。当我们需要弹出堆栈时，位于头部的项目将被移除。
...
我们将[程序 P4.1](#list1) 改写为[程序 P4.2](#list2) 。类别`StackTest`和以前一样，但是类别`Stack`使用了我们对`empty` 、`push`和`pop`的新定义。我们再次强调，即使堆栈的*实现*已经从使用数组变为使用链表，使用堆栈的代码(`main`)仍然保持不变。
