...
图 [14-1](#Fig1) 显示了一个从 NetBeans IDE 启动的简单 JavaFX Hello World 应用。
...
该类也可以在现有的 JDK 8 或 JDK 9 应用中创建。例如，包含本书源代码的项目包含 org . Java 9 recipes . chapter[14](14.html)源代码包中的所有 JavaFX 食谱。这是可能的，因为 JavaFX 不再需要额外的配置；它已经是任何 JDK 8 或 JDK 9 项目的一部分。
...
图 [14-3](#Fig3) 显示了应用于文本节点的各种字体样式和效果(阴影和倒影)的 JavaFX 应用。
...
生成的应用将渲染一个类似于图 [14-4](#Fig4) 所示结果的场景。
...
图 [14-7](#Fig7) 显示了一个模拟的建筑安全应用，包含检查和子菜单项。
...
图 [14-11](#Fig11) 显示了后台进程应用，它模拟了一个文件复制窗口。
...
图 [14-13](#Fig13) 显示了演示 JavaFX 的 TableView 控件的应用。
...
图 [14-15](#Fig15) 显示标签应用，允许用户改变标签方向。
...
图 [14-16](#Fig16) 描述了启用非模态选项的更改密码对话框应用。
