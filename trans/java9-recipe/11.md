...
在本例中，被零除异常发生在 Recipe11_1.java 的第 18 行，是由 main()方法的调用引起的(在第 13 行)。有时，当查看栈跟踪的输出时，您会看到不属于项目的方法。这是自然发生的，因为有时方法调用是在工作系统的其他部分生成的。例如，当出现异常时，在 Swing 应用程序中看到抽象窗口工具包(AWT)方法是很常见的(由于 EventQueue 的性质)。如果您查看更具体的函数调用(最早)，您最终将使用项目自己的代码运行，然后可以尝试确定引发异常的原因。
