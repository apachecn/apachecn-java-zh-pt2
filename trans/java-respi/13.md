...
树莓派OS 不是实时操作系统，Java 也不是实时语言。因此，你*不能*指望用 SilentStepStick 产生真正精确的步进电机*速度*控制，因为 Pi 产生的步进信号受操作系统和 Java 的变化影响。也就是说，你*可以*产生真正精确的步进电机*位置*控制，因为位置只取决于步数，Pi 可以精确控制。
