...
***概述***–*该页面提供了 Axon Server 的* *可视化图形* *以及与之连接的应用实例。到目前为止，由于我们还没有构建任何应用，所以它只描绘了主服务器，如图* [*6-14*](#Fig14) 所示。
...
构建预订绑定上下文应用会产生一个 Spring Boot JAR 文件(bookingms-1.0.jar)。为了启动预订有界上下文应用，我们首先启动 Axon 服务器。然后，我们将预订有界上下文作为常规的 Spring Boot JAR 文件运行。清单 [6-3](#PC3) 对此进行了说明:
...
本章接下来的两个部分将处理货物跟踪应用***-域模型*** 和***-域模型服务*** 的基于 Axon 框架的 DDD 工件的实现。DDD 工件的总体布局如图 [6-30](#Fig30) 所示。
